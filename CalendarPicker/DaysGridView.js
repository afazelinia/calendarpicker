...
export default function DaysGridView(props) {
  ...
  let firstDayOfMonth = startFromMonday ? moment(year+'/'+(month + 1)+'/1', 'jYYYY/jM/jD') : moment(year+'/'+(month + 1)+'/2' , 'jYYYY/jM/jD');
  ...
  const firstDate = moment(year+'/'+(month + 1)+'/1', 'jYYYY/jM/jD').format('jYYYY/jM/jD');
  const secondDate = moment(year+'/'+(month + 1)+'/2' , 'jYYYY/jM/jD').format('jYYYY/jM/jD');
  if (month === 0 && !startFromMonday && firstDate === secondDate) {
    firstDayOfMonth = firstDayOfMonth.add(3, 'hours');
  }
